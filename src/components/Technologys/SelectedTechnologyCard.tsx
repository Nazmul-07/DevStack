...
const SelectedTechnologyCard = ({
  ...
  return (
    <div className="border border-slate-300 rounded-2xl p-4">
        ...
        <p className="mb-5">{`${selectedTech.length} Technology added`}</p>
        {
            selectedTech.length === 0 && <h1 className="flex items-center justify-center border border-dotted border-slate-400 rounded-lg font-semibold py-8">Your Stack is Empty</h1>
        }
      {selectedTech.map((tech) => {
        return (
          <div className="flex items-center justify-between border border-dotted border-slate-300 rounded-lg p-3 mb-3">
            ...
            <TiDelete onClick={()=> handleRemoveBtn(tech)} className="text-2xl text-red-500" />
          </div>
        );
      })}
      {
        selectedTech.length > 0 && <button onClick={()=> setSelectedTech([])} className="w-full mt-7 border border-pink-500 rounded-md py-1 text-pink-500 hover:bg-pink-500 hover:text-slate-50 transition-all duration-300">Remove All</button>
      }
